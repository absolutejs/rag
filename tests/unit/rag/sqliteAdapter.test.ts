...
} from "../../../src/rag/adapters/queryPlanning";
...
describe("createSQLiteRAGStore", () => {
  ...
  it("retrieves nearest chunks with metadata filter support", async () => {
    ...
    expect(filtered.every((hit) => hit.metadata?.tenant === "acme")).toBe(true);
  });
  ...
  it("supports operator-based metadata filters consistently across sqlite and in-memory stores", async () => {
    ...
    for (const store of stores) {
      ...
      expect(missingTags.map((hit) => hit.chunkId).sort()).toEqual(["b", "c"]);
      ...
      expect(boolean.map((hit) => hit.chunkId).sort()).toEqual(["a", "c"]);
      ...
      expect(containsAny.map((hit) => hit.chunkId).sort()).toEqual(["a", "c"]);
  ...
  it("deletes chunks by filters, ids, and unioned criteria", async () => {
    ...
    expect(await store.delete?.({ chunkIds: ["a", "a", "missing"] })).toBe(1);
