...
import type { AIMessage, RAGSource } from "@absolutejs/ai";
import {
...
describe("RAG presentation helpers", () => {
  ...
  it("builds email-specific grounding provenance for messages and attachments", () => {
    ...
    expect(references[0]?.contextLabel).toBe("Message from ops@absolutejs.dev");
  ...
  it("includes pdf and media provenance labels in grounding references", () => {
    ...
    expect(references[2]?.locatorLabel).toBe("Timestamp 00:12.000 - 00:34.500");
  ...
  it("keeps same-name office checklist grounding labels distinct across heading scopes", () => {
    ...
    expect(references[0]?.provenanceLabel).toContain("Office list levels 0-1");
    expect(references[0]?.provenanceLabel).toContain(
    ...
    expect(references[1]?.provenanceLabel).toContain("Office list levels 0-1");
    expect(references[1]?.provenanceLabel).toContain(
  ...
  it("builds source labels from spreadsheet table-local metadata", () => {
    ...
    expect(labels).toMatchObject({
      contextLabel: "Sheet Release Tracker Table 2 of 2 Rows 3-4 Columns C-D",
      locatorLabel:
  ...
  it("builds source labels from email thread lineage metadata", () => {
    ...
    expect(labels).toMatchObject({
      contextLabel: "Attachment evidence checklist.md in Attachment recap",
  ...
  it("builds a first-class chunk graph from section-aware sources", () => {
    const graph = buildRAGChunkGraph([
      buildSource({
        ...
        metadata: {
          sectionChunkCount: 1,
          sectionChunkId: "docs-release-html:section:release-ops-overview",
          sectionChunkIndex: 0,
          sectionDepth: 1,
          sectionKind: "html_heading",
    ...
    expect(graph.sections).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          childSectionIds: ["docs-release-html:section:stable-blockers"],
          chunkCount: 1,
        ...
        expect.objectContaining({
          childSectionIds: [],
          chunkCount: 2,
          chunkIds: ["docs-release-html:001", "docs-release-html:002"],
          id: "docs-release-html:section:stable-blockers",
          leadChunkId: "docs-release-html:001",
          parentSectionId: "docs-release-html:section:release-ops-overview",
          path: ["Release Ops Overview", "Stable blockers"],
          title: "Stable blockers",
        }),
      ]),
  ...
  it("builds grounded answers by resolving citation markers to evidence", () => {
    ...
    expect(grounded.references.map((reference) => reference.number)).toEqual([
      1, 2,
    ]);
    expect(grounded.parts[1]).toMatchObject({
      referenceDetails: [
  ...
  it("builds first-class retrieved state from the latest retrieved message", () => {
    const messages: AIMessage[] = [
      buildAssistantMessage({
        id: "assistant-1",
        sources: [buildSource({ chunkId: "chunk-old", source: "old.md" })],
      }),
      buildAssistantMessage({
        content: "Answer",
        id: "assistant-2",
      ...
      ],
      {
        ...
        steps: [
          ...
          },
          {
            count: 3,
            label:
              "Preferred stronger structured evidence within matching sections",
            metadata: {
              affectedScopes: 1,
              reorderedResults: 2,
            },
    ...
    expect(diagnostics[0]?.stageWeights).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          parentStageScoreShare: expect.closeTo(0.5947712418300654, 12),
        ...
        expect.objectContaining({
          ...
          stageScoreShareGap: expect.closeTo((1.87 - 0.62) / 2.49, 12),
          stageShareGap: 1 / 3,
  ...
  it("prefers presentation slide evidence in grouped source summaries", () => {
    ...
    expect(summaries[0]?.contextLabel).toBe("Slide 3 Release handoff summary");
    ...
    const diagnostics = buildRAGSectionRetrievalDiagnostics([
      buildSource({
        ...
        metadata: {
          ...
          sectionPath: ["Query Attribution Guide", "Regional Growth Workbook"],
      ...
      steps: [
        {
          ...
          metadata: {
            ...
            postgresIndexName: "public_rag_chunks_embedding_hnsw_idx",
