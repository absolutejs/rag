...
import { createRAGFileExtractor } from "../../../src/rag/ingestion";
...
describe("ragChat ingest workflow", () => {
  ...
  it("tracks admin actions in the ops endpoint", async () => {
    ...
    const plugin = ragChat({
      indexManager: {
        ...
        reindexSource(source) {
          const matched = documents.filter((entry) => entry.source === source);
    ...
    expect(
      payload.adminActions.map((entry: { action: string }) => entry.action),
    ).toEqual([
      "clear_index",
      "reset",
      "reseed",
    ...
    const previews = new Map([
      ...
      [
        "doc-2",
        {
          ...
          normalizedText: "Beta workspace incident notes and rollout policy.",
        },
      ],
    ]);
    ...
    const previews = new Map([
      [
        "doc-alpha",
        {
          ...
          normalizedText: "Alpha corpus billing policy and release checklist.",
        },
      ],
      [
        "doc-beta",
        {
          ...
          normalizedText: "Beta corpus incident playbook and rollout notes.",
        },
      ],
    ]);
  ...
  it("lists and runs configured source sync operations", async () => {
    ...
    expect(
      opsPayload.adminActions.map((entry: { action: string }) => entry.action),
    ).toEqual(["sync_all_sources", "sync_source"]);
    expect(
      opsPayload.adminJobs.map((entry: { action: string }) => entry.action),
    ).toEqual(["sync_all_sources", "sync_source"]);
  });
  ...
  it("enriches document chunk previews with source labels from extractor metadata", async () => {
    ...
    const response = await plugin.handle(
      new Request("http://absolute.local/rag/documents/pdf-region-doc/chunks"),
    );
    const payload = await response.json();
  ...
  it("enriches chunk previews with section and adjacency structure", async () => {
    ...
    const response = await plugin.handle(
      new Request("http://absolute.local/rag/documents/section-doc/chunks"),
    );
    const payload = await response.json();
  ...
  it("enriches chunk previews with block-aware pdf labels and structure", async () => {
    ...
    const response = await plugin.handle(
      new Request("http://absolute.local/rag/documents/pdf-block-doc/chunks"),
    );
    const payload = await response.json();
