...
describe("createRAGClient", () => {
  it("calls search and returns normalized results", async () => {
    const fetchMock = (async (input, init) => {
      ...
      expect(init?.body).toBe(JSON.stringify({ query: "hello", topK: 2 }));
  ...
  it("loads persisted search trace history through the client", async () => {
    ...
    expect(history).toEqual(
      expect.objectContaining({
        ...
        query: "alpha",
        tag: "guide",
        traces: [expect.objectContaining({ id: "trace-1", query: "alpha" })],
      }),
    );
  });
  ...
  it("loads prune run history through the client", async () => {
    const fetchMock = (async (input) => {
      expect(input).toBe("/rag/traces/prune/history?limit=5&trigger=manual");
    ...
    const fetchMock = (async (input, init) => {
      ...
      return new Response(
        JSON.stringify({
          ok: true,
          comparison: {
            entries: [{ retrievalId: "vector" }, { retrievalId: "lexical" }],
            summary: { bestByPassingRate: "lexical" },
          },
        }),
        { status: 200 },
  ...
  it("promotes, reverts, and lists retrieval baselines through the client", async () => {
        ...
        return new Response(
          JSON.stringify({
            ...
            rolloutState: {
              ...
              reasons: ["baseline is active in the target rollout lane"],
            },
          }),
          { status: 200 },
        );
          ...
          return new Response(
            JSON.stringify({
              ...
              rolloutState: {
                ...
                reasons: ["explicit approval is required before promotion"],
                requiresApproval: true,
                targetRolloutLabel: "stable",
              },
            }),
              ...
              decisions: [
                {
                  ...
                  kind: "reject",
                  notes:
                    "stable handoff is blocked until gate and approval pass",
                  sourceRolloutLabel: "canary",
                  targetRolloutLabel: "stable",
                },
              ],
    ...
    expect(calls[0]?.input).toBe("/rag/compare/retrieval/baselines/promote");
    expect(calls[0]?.method).toBe("POST");
    expect(calls[1]?.input).toBe("/rag/compare/retrieval/baselines/promote");
    ...
    expect(calls[32]?.input).toBe("/rag/compare/retrieval/incidents/resolve");
    expect(calls[32]?.method).toBe("POST");
    expect(calls[33]?.input).toBe("/rag/compare/retrieval/baselines/approve");
    expect(calls[33]?.method).toBe("POST");
    expect(calls[34]?.input).toBe("/rag/compare/retrieval/baselines/reject");
  ...
  it("loads and persists adaptive native planner benchmark governance routes through the client", async () => {
    ...
    const client = createRAGClient({
      fetch: (async (input, init) => {
        ...
        if (
          String(input).includes("/benchmarks/native-backend-comparison/run")
        ) {
          return new Response(
            JSON.stringify({
        ...
        if (String(input).includes("/benchmarks/native-backend-comparison")) {
          return new Response(
            JSON.stringify({
              groupKey: "runtime-native-backend-parity",
              ok: true,
        ...
        if (String(input).includes("/benchmarks/adaptive-native-planner/run")) {
          return new Response(
            JSON.stringify({
              fixtureVariants: ["current-collection"],
              comparison: {
        ...
        if (String(input).includes("/benchmarks/adaptive-native-planner")) {
          return new Response(
            JSON.stringify({
              groupKey: "runtime-native-planner",
              ok: true,
  ...
  it("loads lightweight retrieval release status helpers from the workflow endpoint", async () => {
    ...
    const releaseIncidentStatus = await client.retrievalReleaseIncidentStatus();
    const remediationStatus = await client.retrievalIncidentRemediationStatus();
    ...
    expect(releaseStatus?.recentHandoffAutoCompletePolicyHistory?.[0]).toEqual(
      expect.objectContaining({
        changeKind: "snapshot",
        groupKey: "docs-release",
        targetRolloutLabel: "stable",
    ...
    expect(remediationStatus.recentIncidentRemediationExecutions?.[0]).toEqual(
      expect.objectContaining({
        code: "guardrail_blocked",
      }),
    );
  ...
  it("loads indexed documents from the workflow endpoint", async () => {
    ...
    expect(response.documents[0]?.labels?.locatorLabel).toBe("Sheet Overview");
  });
  ...
  it("loads block-aware document chunk previews from the workflow endpoint", async () => {
    ...
    if (response.ok) {
      ...
      expect(response.chunks[0]?.structure?.section?.kind).toBe("pdf_block");
    }
  });
