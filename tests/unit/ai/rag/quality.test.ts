...
describe("RAG quality helpers", () => {
  ...
  it("classifies retrieval and grounding failure modes explicitly", () => {
    ...
    expect(retrievalFail.failureClasses).toEqual(["no_results", "no_match"]);
  ...
  it("builds per-source and per-document quality views from failure-classified results", () => {
    ...
    const evaluationPresentation = buildRAGEvaluationEntityQualityPresentation(
      evaluationView,
      "source",
    );
    ...
    const groundingPresentation =
      buildRAGAnswerGroundingEntityQualityPresentation(groundingView, "source");
    expect(
  ...
  it("keeps active baselines separate per rollout lane", async () => {
    ...
    expect(
      activeBaselines.find((entry) => entry.rolloutLabel === "stable")?.status,
    ).toBe("active");
    expect(
      activeBaselines.find((entry) => entry.rolloutLabel === "canary")?.status,
    ).toBe("active");
  });
  ...
  it("scores grounded answers for citation fidelity against expected sources", () => {
    const result = evaluateRAGAnswerGroundingCase({
      ...
      caseInput: {
        answer:
          "The PDF policy stays inspectable on page 7 [1], and the spreadsheet keeps the Regional Growth sheet named explicitly [2].",
  ...
  it("tracks unresolved citations and partial grounding accurately", () => {
    const result = evaluateRAGAnswerGroundingCase({
      ...
      caseInput: {
        answer:
          "The answer cites the PDF correctly [1], but it also references an unresolved source [3].",
        expectedSources: ["docs/guide.pdf", "docs/report.xlsx"],
  ...
  it("loads grouped search trace history by group key and tag", async () => {
    ...
    expect(
      docsGroups.groups[0]?.retrievalTraceTrend.latestToPrevious?.modeDelta
        .added,
    ).toEqual(["hybrid"]);
  ...
  it("persists search trace prune history in SQLite-backed stores", async () => {
    ...
    expect(runs.map((entry) => `${entry.trigger}:${entry.finishedAt}`)).toEqual(
      ["manual:2", "schedule:1"],
    );
  ...
  it("prunes file-backed search trace stores by age and retention caps", async () => {
    ...
    expect(docsHistory.traces.map((trace) => trace.id)).toEqual(["prune-3"]);

    const supportHistory = await loadRAGSearchTraceHistory({
      query: "reset password",
      store,
    });
    expect(supportHistory.traces.map((trace) => trace.id)).toEqual(["prune-4"]);
  });
  ...
  it("runs saved suites and builds a leaderboard", async () => {
    const suite = createRAGEvaluationSuite({
      id: "core-suite",
      input: {
        cases: [{ expectedDocumentIds: ["a"], id: "case-a", query: "alpha" }],
      },
      label: "Core Suite",
    });
    ...
    const leaderboard = buildRAGEvaluationLeaderboard([weakerRun, strongRun]);
  ...
  it("supports immutable evaluation suite authoring helpers", () => {
    ...
    expect(
      withoutHardNegative.input.cases.find((entry) => entry.id === "case-b"),
    ).toMatchObject({
      goldenSet: true,
  ...
  it("persists evaluation and grounding histories through SQLite stores", async () => {
    ...
    const groundingStore = createRAGSQLiteAnswerGroundingEvaluationHistoryStore(
      { db },
    );
    const snapshotStore = createRAGSQLiteEvaluationSuiteSnapshotHistoryStore({
      db,
    });
  ...
  it("applies uniform retention pruning across file and SQLite evaluation stores", async () => {
    ...
    expect(await fileStore.listRuns({ suiteId: "suite-a" })).toHaveLength(1);
  ...
  it("builds diffs and persisted history for suite runs", async () => {
    ...
    const singleRunStore = createRAGFileEvaluationHistoryStore(singleRunPath);
  ...
  it("builds leaderboards and persisted history for grounding runs", async () => {
    ...
    const groundingHistoryRows = buildRAGAnswerGroundingHistoryRows(history);
  ...
  it("builds backend-specific larger-corpus benchmark fixtures", async () => {
    ...
    expect(
      postgres.find((entry) => entry.chunkId === "focus:postgres:appendix")
        ?.metadata,
    ).toMatchObject({
      ...
      input: expect.objectContaining({
        cases: expect.arrayContaining([
          expect.objectContaining({
            ...
            query:
              "Which presentation title covers the release handoff summary?",
          }),
          expect.objectContaining({
  ...
  it("builds diffs and persisted history for grounding difficulty runs", async () => {
    ...
    tempPaths.add(path);
    const store = createRAGFileAnswerGroundingCaseDifficultyHistoryStore(path);
  ...
  it("evaluates a collection and compares rerankers on the same suite", async () => {
    ...
    expect(
      comparison.entries.find(
        (entry) => entry.rerankerId === comparison.summary.bestByPassingRate,
  ...
  it("improves lexical-heavy benchmark cases with hybrid retrieval", async () => {
    ...
    const vectorResponse = await evaluateRAGCollection({
      collection: {
        ...collection,
        search: (input) => collection.search({ ...input, retrieval: "vector" }),
      },
      input: {
        cases: [
          {
    ...
    const hybridResponse = await evaluateRAGCollection({
      collection: {
        ...collection,
        search: (input) => collection.search({ ...input, retrieval: "hybrid" }),
      },
      input: {
        cases: [
          {
  ...
  it("improves multivector exact-phrase benchmark cases with heuristic retrieval routing", async () => {
    ...
    expect(heuristicTrace.trace.routingLabel).toBe("Exact phrase hybrid route");
  ...
  it("improves larger-corpus native benchmark cases with adaptive planner selection", async () => {
    ...
    expect(seen.some((input) => input.plannerProfile === "latency")).toBe(true);
    expect(seen.some((input) => input.plannerProfile === "recall")).toBe(true);
    expect(adaptiveTrace.trace.steps).toContainEqual(
      expect.objectContaining({
        label: "Selected native planner profile",
        metadata: expect.objectContaining({
  ...
  it("surfaces multivector contribution winners in retrieval comparisons", async () => {
    ...
    expect(comparison.summary.bestByMultivectorCollapsedCases).toBe("hybrid");
    expect(comparison.summary.bestByMultivectorLexicalHitCases).toBe("hybrid");
    expect(comparison.summary.bestByMultivectorVectorHitCases).toBe("hybrid");
  });
  ...
  it("applies presentation cue thresholds in retrieval comparison gates", () => {
    ...
    expect(decisionSummary?.winnerByPresentationBodyCueCases).toBe("cue-aware");
  ...
  it("persists retrieval release decisions for promotion and rollback audit trails", async () => {
    ...
    expect(decisions.map((entry) => `${entry.kind}:${entry.version}`)).toEqual([
      "revert:2",
      "promote:1",
    ]);
  ...
  it("persists retrieval release incidents for operational alerting", async () => {
    ...
    expect(incidents.map((entry) => `${entry.kind}:${entry.status}`)).toEqual([
      "gate_failure:open",
      "approval_expired:resolved",
    ]);
  ...
  it("persists retrieval lane handoff incident history in SQLite-backed stores", async () => {
    ...
    const acknowledgements = await loadRAGRetrievalLaneHandoffIncidentHistory({
      action: "acknowledged",
      groupKey: "docs-release",
      limit: 5,
      store,
  ...
  it("persists incident remediation execution history in SQLite-backed stores", async () => {
    ...
    const records = await loadRAGRetrievalIncidentRemediationExecutionHistory({
      groupKey: "docs-release",
      incidentId: "incident-1",
      limit: 5,
      store,
    ...
    expect(records.map((entry) => `${entry.code}:${entry.executedAt}`)).toEqual(
      ["incident_resolved:2", "guardrail_blocked:1"],
    );
  ...
  it("surfaces source-balance strategy differences in retrieval comparisons", async () => {
    ...
    expect(
      comparison.entries.find((entry) => entry.retrievalId === "round-robin")
        ?.traceSummary?.sourceBalanceStrategies,
    ).toEqual(["round_robin"]);
    expect(
      comparison.entries.find((entry) => entry.retrievalId === "round-robin")
  ...
  it("can benchmark MMR diversity retrieval as a distinct strategy", async () => {
    ...
    expect(
      comparison.entries.find((entry) => entry.retrievalId === "mmr")?.response
        .summary.passedCases,
  ...
  it("builds reusable comparison and history rows for trace-aware evaluation UIs", async () => {
    ...
    const retrievalCards = buildRAGRetrievalComparisonPresentations(comparison);
    ...
    expect(retrievalOverview).toMatchObject({
      winnerLabel:
        comparison.entries.find(
          (entry) => entry.retrievalId === comparison.summary.bestByPassingRate,
        )?.label ?? "n/a",
    });
    ...
    const diffRows = buildRAGComparisonTraceDiffRows(vectorEntry, hybridEntry);
    ...
    const historyPresentation = buildRAGEvaluationHistoryPresentation(history);
  ...
  it("uses media segment duration metadata for continuous/long timeline reranking decisions", async () => {
    ...
    const longQuery = await collection.search({
      query:
        "Which continuous audio timestamp has the long duration and no gap where review happens?",
    });
  ...
  it("uses media segment gap metadata for continuity-focused reranking", async () => {
    ...
    const gapQuery = await collection.search({
      query:
        "Which continuous media segment has no gap between segments in this timeline?",
    });
  ...
  it("does not reward malformed media timing metadata in continuity reranking", async () => {
    ...
    const continuityQuery = await collection.search({
      query:
        "Which long continuous media segment has no gap and has a long duration?",
    });
