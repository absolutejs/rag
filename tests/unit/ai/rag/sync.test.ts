...
describe("RAG sync helpers", () => {
  ...
  it("threads extractor and chunking registries through directory sync sources", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "absolute-rag-sync-registry-"));
    const store = createInMemoryRAGStore({ dimensions: 8 });
    const collection = createRAGCollection({ store });
    ...
    try {
      ...
      expect(hits[0]?.metadata?.extractor).toBe("demo_note_sync_extractor");
      expect(sectionedHit?.metadata?.sectionTitle).toBeDefined();
      expect(sectionedHit?.metadata?.sectionKind).toBe("markdown_heading");
    } finally {
      rmSync(tempDir, { force: true, recursive: true });
    }
  });
          ...
          createRAGDirectorySyncSource({
            ...
            extractors: [
              createRAGFileExtractor({
                ...
                supports: (input) =>
                  String(input.path ?? input.source ?? "").endsWith(".bin"),
                extract() {
                  throw new Error(
  ...
  it("reconciles removed directory documents through list/delete hooks", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "absolute-rag-sync-reconcile-"));
    const store = createInMemoryRAGStore({ dimensions: 8 });
    const collection = createRAGCollection({ store });
  ...
  it("stamps refreshed sync documents with version lineage metadata", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "absolute-rag-sync-lineage-"));
    const store = createInMemoryRAGStore({ dimensions: 8 });
    const collection = createRAGCollection({ store });
    ...
    try {
      ...
      expect(hits[0]?.metadata?.syncPreviousVersionId).toBe(previousVersionId);
      expect(hits[0]?.metadata?.syncPreviousDocumentId).toBe("guide-md");
      expect(hits[0]?.metadata?.syncVersionId).not.toBe(previousVersionId);
  ...
  it("surfaces duplicate sync-key and lineage conflicts in reconciliation summaries", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "absolute-rag-sync-conflicts-"));
    const store = createInMemoryRAGStore({ dimensions: 8 });
    const collection = createRAGCollection({ store });
        ...
        source: {
          ...
          reconciliation: {
            ...
            lineageConflicts: [
              {
                documentIds: ["guide-md-v2a", "guide-md-v2b"],
                latestDocumentIds: ["guide-md-v2a", "guide-md-v2b"],
                lineageIds: [
    ...
    const reconciliation: RAGSyncSourceReconciliationSummary = {
      ...
      lineageConflicts: [
        ...
        },
        {
          ...
          lineageIds: ["docs-folder:faq.md", "docs-folder:faq.md-alt"],
          reasons: [
            "duplicate_sync_key",
            "multiple_lineages",
            "multiple_versions",
  ...
  it("auto-discovers robots sitemaps and follows sitemap indexes before ingesting URLs", async () => {
    ...
    globalThis.fetch = createMockFetchMap({
      ...
      "https://example.com/sitemap.xml": new Response("<urlset></urlset>", {
        headers: { "content-type": "application/xml" },
        status: 200,
      }),
  ...
  it("discovers bounded same-origin linked pages from site-discovery seeds without creating a crawler subsystem", async () => {
    ...
    try {
      ...
      expect(allSources).not.toContain("https://outside.example.com/ignored");
    } finally {
      globalThis.fetch = fetchOriginal;
    }
  });
  ...
  it("respects canonical URLs and basic robots restrictions during bounded linked-page discovery", async () => {
    ...
    try {
      ...
      if (!response || !response.ok || !("source" in response)) {
        throw new Error("Expected a successful single-source sync response");
      }
      expect(
        response.source.diagnostics?.entries.map((entry) => entry.code) ?? [],
      ).toEqual(
      ...
      expect(allSources).not.toContain("https://example.com/private/secret");
    } finally {
      globalThis.fetch = fetchOriginal;
    }
  });
  ...
  it("dedupes canonical page variants and skips noindex pages during bounded linked-page discovery", async () => {
    ...
    try {
      ...
      expect(
        ...
        expect.arrayContaining(["canonical_dedupe_applied", "noindex_skipped"]),
      );
        ...
        createRAGEmailSyncSource({
          ...
          extractors: [
            createRAGFileExtractor({
              ...
              supports: (input) =>
                String(input.name ?? input.source ?? "").endsWith(".pdf"),
              extract() {
                throw new Error(
      ...
      sources: [
        createRAGStorageSyncSource({
          client: {
            file: () => ({
              arrayBuffer: async () =>
                new Uint8Array(Buffer.from('tenant,status\nacme,"ready'))
                  .buffer,
            }),
            list: async () => ({
        ...
        diagnostics: {
          extractionFailures: [
            expect.objectContaining({
              ...
              reason: expect.stringContaining("malformed CSV at line 2"),
            }),
          ],
          retryGuidance: {
      ...
      diagnostics: {
        ...
        extractionFailures: [
          ...
          },
          {
            ...
            reason:
              "No RAG file extractor matched legacy.dat. Register a custom extractor for this file type.",
            remediation: "configure_extractor",
          },
        ],
  ...
  it("returns partial sync results when one source fails and keeps successful sources", async () => {
    const store = createInMemoryRAGStore({ dimensions: 8 });
    const collection = createRAGCollection({ store });
    const tempDir = mkdtempSync(join(tmpdir(), "absolute-rag-sync-partial-"));
  ...
  it("hydrates persisted sync state and saves new records after sync runs", async () => {
    const store = createInMemoryRAGStore({ dimensions: 8 });
    const collection = createRAGCollection({ store });
    const tempDir = mkdtempSync(join(tmpdir(), "absolute-rag-sync-persist-"));
  ...
  it("persists sync state records to a file-backed store", async () => {
    ...
    const store = createRAGFileSyncStateStore(join(tempDir, "sync-state.json"));
  ...
  it("falls back to empty job state when the file-backed job store is corrupted", async () => {
    ...
    const store = createRAGFileJobStateStore(join(tempDir, "job-state.json"));
