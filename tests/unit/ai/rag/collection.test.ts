...
const buildRepeatedClosureOfficeMetadata = ({
  ...
  const closureTitle =
    branchOrdinal === 1 ? "Closure Notes" : `Closure Notes (${branchOrdinal})`;
  const familyTitle =
...
const assertMultivectorRerankParity = async ({
  ...
  expect(traced.results[0]?.metadata).toEqual(
    expect.objectContaining({
      ...
      multivectorMatchedVariantText: "aurora launch packet sign-off checklist",
    }),
  );
...
describe("createRAGCollection", () => {
  ...
  it("prefers native-layout hybrid pdf evidence over ocr-only page hits within the same page scope", async () => {
    ...
    expect(
      traced.trace.steps.find((step) => step.stage === "evidence_reconcile"),
    ).toMatchObject({
  ...
  it("prefers deeper repeated-scope office table evidence within the same source and title", async () => {
    ...
    expect(
      ...
      label:
        "Preferred deeper office-structure evidence within matching sections",
      metadata: {
        affectedScopes: 2,
      ...
      query: async () => [
        {
          ...
          chunkText: "Spreadsheet table summary with shallow lineage.",
          metadata: {
        ...
        },
        {
          ...
          chunkText: "Spreadsheet table summary with workbook-scoped lineage.",
          metadata: {
            sectionKind: "spreadsheet_rows",
            sectionFamilyPath: [
              "Release Tracker",
    ...
    const store: RAGVectorStore = {
      ...
      query: async () => [
        ...
        },
        {
          ...
          chunkText: "Stable second sibling closure checklist evidence.",
          metadata: {
            officeBlockKind: "list",
            sectionKind: "office_block",
            sectionPath: [
  ...
  it("collapses multivector variant hits back to a single parent chunk", async () => {
    ...
    expect(
      Number(
        traced.trace.steps.find((step) => step.stage === "vector_search")
          ?.metadata?.multiVectorVariantHits ?? 0,
      ),
    ).toBeGreaterThanOrEqual(1);
    expect(
      Number(
        traced.trace.steps.find((step) => step.stage === "lexical_search")
          ?.metadata?.multiVectorVariantHits ?? 0,
      ),
    ).toBeGreaterThanOrEqual(1);
  });
      ...
      traced.results.every(
        (result) =>
          ...
          (result.metadata?.retrievalQueryOrigin === "primary" ||
            result.metadata?.retrievalQueryOrigin === "transformed" ||
  ...
  itIfPostgres(
    ...
    async () => {
      ...
      expect(vectorStage?.metadata).toMatchObject({
        ...
        postgresIndexName: expect.stringContaining("_embedding_hnsw_idx"),
  ...
  it("supports retrieval strategy selectors that route queries per request", async () => {
    ...
    expect(
      traced.trace.steps.find((step) => step.stage === "routing")?.metadata,
    ).toEqual(
      expect.objectContaining({
        providerName: "test_router",
  ...
  it("prefers spreadsheet chunks whose column span matches column-oriented queries", async () => {
    ...
    const results = await collection.search({
      query:
        "Which spreadsheet table covers columns C and D for owner due date?",
      topK: 1,
    });
  ...
  it("prefers the right presentation cue family for title, body, and notes queries", async () => {
    ...
    expect(
      titleTrace.trace.steps.find((step) => step.stage === "rerank")?.metadata
        ?.leadPresentationCue,
    ).toBe("title");
    expect(bodyTrace.results[0]?.chunkId).toBe("slide-body");
    expect(
      bodyTrace.trace.steps.find((step) => step.stage === "rerank")?.metadata
        ?.leadPresentationCue,
    ).toBe("body");
    expect(notesTrace.results[0]?.chunkId).toBe("slide-notes");
    expect(
      notesTrace.trace.steps.find((step) => step.stage === "rerank")?.metadata
        ?.leadPresentationCue,
    ).toBe("notes");
  });
  ...
  it("prefers email thread-local evidence for reply-oriented heuristic reranking", async () => {
    ...
    const results = await collection.search({
      query:
        "Which email thread attachment contains the recap checklist reply?",
      topK: 1,
    });
  ...
  it("prefers richer email thread-chain evidence for reply-oriented heuristic reranking", async () => {
    ...
    const results = await collection.search({
      query:
        "Which email thread ancestor chain reply explains the older attachment recap lineage?",
      topK: 1,
    });
  ...
  it("prefers authored email body evidence over quoted and forwarded sections for generic reply queries", async () => {
    ...
    const results = await collection.search({
      query:
        "What does the latest email reply actually say in the incident thread summary?",
      topK: 3,
    });
  ...
  it("prefers forwarded email header evidence for sender-oriented forwarded queries", async () => {
    ...
    const results = await collection.search({
      query:
        "Which forwarded email headers show the original sender in the incident chain?",
      topK: 1,
    });
  ...
  it("uses attached-message lineage depth instead of hardcoded email nesting levels", async () => {
        ...
        metadata: {
          emailMessageLineage: Array.from(
            ...
            (_, lineageIndex) => ({
              ...
              messageSourceKind:
                lineageIndex === 0 ? "root_message" : "attached_message",
            }),
          ),
    ...
    const results = await collection.search({
      query:
        "Which nested attached forwarded email in the chain contains the ancestry context?",
      topK: 5,
    });
  ...
  it("prefers authored attached-email evidence over forwarded headers within matching sibling branches", async () => {
    ...
    const results = await collection.search({
      query:
        "Which branch-a attached email reply has the actual local authored summary?",
      topK: 2,
    });
  ...
  it("prefers the right sibling branch and email section family across authored, quoted, and forwarded evidence", async () => {
    ...
    const authoredResults = await collection.search({
      query:
        "Which branch-a attached email has the actual authored escalation summary?",
      topK: 1,
    });
    const quotedResults = await collection.search({
      query:
        'Which lantern attached email quoted history shows "Lantern-only quoted escalation note for branch B"?',
      topK: 1,
    });
    const forwardedResults = await collection.search({
      query:
        "Which branch-a attached email forwarded headers show the original sender?",
      topK: 1,
    });
  ...
  it("prefers mailbox-local email evidence when mailbox container and folder cues match the query", async () => {
    ...
    const results = await collection.search({
      query:
        "Which lantern mailbox folder reply has the actual local incident summary?",
      topK: 1,
    });
  ...
  it("prefers the right mailbox family and email section under maildir thread collisions", async () => {
    ...
    const authoredResults = await collection.search({
      query:
        "Which Lantern maildir mailbox folder has the actual local incident summary?",
      topK: 1,
    });
    const forwardedResults = await collection.search({
      query:
        "Which Quartz maildir mailbox folder forwarded headers show the original sender?",
      topK: 1,
    });
  ...
  it("prefers the right nested mailbox family using mailbox lineage metadata instead of hardcoded folder levels", async () => {
    ...
    const results = await collection.search({
      query:
        "Which nightly escalations maildir mailbox folder has the authored summary?",
      topK: families.length,
    });

    expect(results[0]?.chunkId).toBe("mailbox-family-4");
    expect(results[1]?.chunkId).toBe("mailbox-family-3");
    expect(new Set(results.slice(2).map((result) => result.chunkId))).toEqual(
      new Set(["mailbox-family-1", "mailbox-family-2"]),
    );
  });
  ...
  it("prefers mailbox-state-matching email evidence within the same mailbox family", async () => {
    ...
    const passedResults = await collection.search({
      query:
        "Which passed ops release maildir reply has the local authored summary?",
      topK: 1,
    });
    const flaggedResults = await collection.search({
      query:
        "Which flagged ops release maildir reply has the local authored summary?",
      topK: 1,
    });
    const draftResults = await collection.search({
      query:
        "Which draft ops release maildir reply has the local authored summary?",
      topK: 1,
    });
  ...
  it("prefers ost mailbox-state and folder-local evidence within one mailbox container", async () => {
    ...
    const flaggedResults = await collection.search({
      query:
        "Which flagged inbox west ost mailbox folder has the authored summary?",
      topK: 1,
    });
    const passedResults = await collection.search({
      query:
        "Which passed archive west ost mailbox folder has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox importance and category-local evidence within one mailbox container", async () => {
    ...
    const importanceResults = await collection.search({
      query:
        "Which high priority pst escalation message has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox message-class and conversation-index evidence within one mailbox container", async () => {
    ...
    const classResults = await collection.search({
      query:
        "Which pst mailbox note class message has the authored summary for lantern regional recovery?",
      topK: 1,
    });
    ...
    const conversationIndexResults = await collection.search({
      query:
        "Which pst mailbox conversation index lantern conv 11 message has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox normalized-subject evidence within one mailbox container", async () => {
      ...
      chunks: [
        {
          ...
          metadata: {
            emailMailboxContainerSource: "lantern-thread.pst",
            emailMailboxFormat: "pst",
            emailNormalizedSubject: "Lantern regional recovery followup",
            emailSectionKind: "authored_text",
            sectionKind: "email_block",
            threadTopic: "Lantern regional recovery",
          },
    ...
    const results = await collection.search({
      query:
        "Which pst mailbox normalized subject quartz incident fallback has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox conversation-id evidence within one mailbox container", async () => {
    ...
    const results = await collection.search({
      query:
        "Which pst mailbox conversation id beta 002 has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox timestamp cues within one mailbox container", async () => {
    ...
    const deliveredResults = await collection.search({
      query:
        "Which pst delivered message has the authored summary for lantern regional recovery?",
      topK: 1,
    });
    ...
    const modifiedResults = await collection.search({
      query:
        "Which pst modified message has the authored summary for lantern regional recovery?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox internet-message-id evidence within one mailbox container", async () => {
      ...
      chunks: [
        {
          ...
          embedding: [1, 0],
          metadata: {
            emailInternetMessageId: "<lantern-route-77@example.com>",
            emailMailboxContainerSource: "lantern-thread.pst",
            emailMailboxFormat: "pst",
            emailSectionKind: "authored_text",
            sectionKind: "email_block",
    ...
    const messageIdResults = await collection.search({
      query:
        "Which pst internet message id lantern route 77 message has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox message-id evidence within one mailbox container", async () => {
    ...
    const messageIdResults = await collection.search({
      query:
        "Which pst message id lantern raw route 31 message has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox thread-index evidence within one mailbox container", async () => {
    ...
    const threadIndexResults = await collection.search({
      query:
        "Which pst thread index AQHTHREAD.001 message has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox reply-parent evidence within one mailbox container", async () => {
    ...
    const parentResults = await collection.search({
      query:
        "Which pst reply parent lantern root message has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox reference-chain evidence within one mailbox container", async () => {
    ...
    const referenceResults = await collection.search({
      query:
        "Which pst reference chain lantern reference anchor has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers pst mailbox thread-root evidence within one mailbox container", async () => {
    ...
    const rootResults = await collection.search({
      query:
        "Which pst thread root lantern root message has the authored summary?",
      topK: 1,
    });
  ...
  it("prefers deeper quoted email history when the query explicitly asks for older thread context", async () => {
    ...
    const quotedDepthResults = await collection.search({
      query:
        "Which deeper older quoted history has the prior escalation archive context for the email thread?",
      topK: 1,
    });
  ...
  it("prefers inline email attachment ordinals within one attached-message branch", async () => {
      ...
      chunks: [
        {
          ...
          metadata: {
            ...
            emailMessageLineageCount: 1,
            emailMessageSource: "incident.eml#attachments/child.eml",
            emailMessageSourceKind: "attached_message",
    ...
    const secondInlineResults = await collection.search({
      query:
        "Which second inline cid resource shows the badge preview note for the attached email branch?",
      topK: 1,
    });
  ...
  it("prefers the right recovered pst attachment branch within one mailbox container", async () => {
    ...
    const results = await collection.search({
      query:
        "Which second pst recovered guide attachment is in the second mailbox branch?",
      topK: 1,
    });
          ...
          return [
            {
              ...
              metadata: {
                ...
                emailSectionKind: "authored_text",
                sectionKind: "email_block",
                threadTopic: "Recovered PST attached descendant branches",
              },
      ...
      chunks: Array.from({ length: caseCount }, (_, index) => {
        ...
        return familyKeys.flatMap((familyKey) =>
          Array.from({ length: depthCount }, (_, depthIndex) => {
            ...
            const lineageAttachmentSources = buildLineageAttachmentSources({
              caseKey,
              depth,
              familyKey,
              ordinal,
            ...
            const attachedMessageSource =
              lineageAttachmentSources[lineageAttachmentSources.length - 1]!;
            return {
              ...
              metadata: {
                ...
                emailMessageLineageAttachmentSources: lineageAttachmentSources,
      ...
      for (const familyKey of familyKeys) {
        for (const branchKey of branchKeys) {
          const results = await collection.search({
            ...
            topK:
              caseCount * familyKeys.length * branchKeys.length * depthCount,
          });
          ...
          Array.from({ length: replyCount }, (_, replyIndex) => {
            ...
            return {
              ...
              metadata: {
                ...
                emailSectionKind: "authored_text",
                sectionKind: "email_block",
                threadTopic: "Recovered PST descendant reply sibling branches",
              },
  ...
  it("prefers replicated recovered descendant families by mailbox container and state instead of shared attachment names", async () => {
    ...
    const containerSpecs = MIXED_MAILBOX_REPLY_SPECS.filter(
      ...
      containerSource: replicatedContainerSourceByFormat[spec.formatLabel],
      familyKey: mixedMailboxFamilyKey(MIXED_SHARED_MAILBOX_PATH_SEGMENTS),
    ...
    await collection.ingest({
      chunks: containerSpecs.map((spec) => ({
        ...
        metadata: {
          ...
          emailMailboxPathDepth: 3,
          emailMailboxPathSegments: MIXED_SHARED_MAILBOX_PATH_SEGMENTS,
    ...
    await collection.ingest({
      chunks: containerSpecs.flatMap((spec) => [
        ...
        },
        {
          ...
          embedding: [1, 0] as [number, number],
          metadata: {
            emailForwardedFromAddress: "original-sender@example.com",
      ...
      chunks: replySpecs.flatMap((spec) => {
        ...
        return [
          ...
          },
          {
            ...
            metadata: {
              emailForwardedFromAddress: "original-sender@example.com",
              emailForwardedSubject: "Mixed container sibling history",
              emailMailboxContainerSource: spec.containerSource,
              emailMailboxFamilyKey: "ops/recovered/shared",
              emailMailboxFolder: mixedMailboxFolder(spec.formatLabel),
              emailMailboxFormat: spec.formatLabel,
  ...
  it("prefers the right mixed-format attached-message descendant by ordinal format and section family", async () => {
    ...
    const replySpecs = MIXED_MAILBOX_REPLY_SPECS.map((spec) => ({
      ...
      containerSource: spec.containerSource,
      expectedChildSource: expectedDescendantSourceByFormat[spec.formatLabel],
      formatLabel: spec.formatLabel,
  ...
  it("prefers arbitrary mixed-format attached-message child branches by branch ordinal format and section family", async () => {
    ...
    await collection.ingest({
      chunks: replySpecs.flatMap((spec) =>
        branchKeys.flatMap((branchKey) => {
          const childSource = mixedMailboxExpectedChildSource(spec, branchKey);
          ...
          return [
            {
              ...
              metadata: {
                ...
                emailReplySiblingOrdinal: expectedIndex + 1,
                emailSectionKind: "authored_text",
                threadTopic: "Mixed container descendant branches thread",
              },
      ...
      for (const branchKey of branchKeys) {
        ...
        const statePrefix = spec.stateFlags.includes("unread")
          ...
          : spec.stateFlags.includes("flagged")
            ? "flagged "
            : "";
        const ordinalLabel = ordinalLabelBySource.get(childSource) ?? "reply";
          ...
          return [
            {
              ...
              metadata: {
                ...
                emailMessageSourceKind: "attached_message",
                emailSectionKind: "authored_text",
                threadTopic: "Mixed container descendant nested thread",
              },
    ...
    await collection.ingest({
      chunks: replySpecs.flatMap((spec) =>
        branchKeys.flatMap((branchKey) => {
          ...
          return nestedReplyOrdinals.flatMap((replyOrdinal, index) => {
        ...
        for (const replyOrdinal of nestedReplyOrdinals) {
          const statePrefix = spec.stateFlags.includes("unread")
            ...
            : spec.stateFlags.includes("flagged")
              ? "flagged "
              : "";
          const ordinalLabel = replyOrdinal === 1 ? "first" : "second";
          const authoredResults = await collection.search({
  ...
  it("prefers nested archive locality evidence for nested archive queries", async () => {
    ...
    const results = await collection.search({
      query:
        "Which nested inner archive entry contains the guide recovery notes?",
      topK: 1,
    });
  ...
  it("prefers higher-confidence OCR evidence for OCR-oriented heuristic reranking", async () => {
    ...
    const results = await collection.search({
      query:
        "Which OCR region on the scanned page explains the recovery procedure?",
      topK: 1,
    });
  ...
  it("prefers media transcript segments over generic workflow chunks in lexical retrieval", async () => {
    ...
    const results = await collection.search({
      query:
        "Which source says the workflow stays aligned across every frontend?",
      retrieval: "lexical",
      topK: 1,
    });
  ...
  it("prefers media timestamp evidence for timestamp-oriented lexical queries", async () => {
    ...
    const results = await collection.search({
      query:
        "Which audio timestamp says retrieval, citations, evaluation, and ingest workflows stay aligned across every frontend?",
      retrieval: "lexical",
      topK: 1,
    });
  ...
  it("prefers speaker-rich media segment evidence for speaker-oriented heuristic reranking", async () => {
    ...
    const results = await collection.search({
      query:
        "Which speaker says the workflow stays aligned across every frontend?",
      topK: 1,
    });
  ...
  it("prefers channel-targeted media evidence for channel-oriented heuristic reranking", async () => {
    ...
    const results = await collection.search({
      query:
        "Which left channel says the workflow stays aligned across every frontend?",
      topK: 1,
    });
  ...
  it("prefers continuous, long media segment windows for timeline-oriented heuristic reranking", async () => {
    ...
    const results = await collection.search({
      query:
        "Which continuous audio timestamp has the long duration and no gap in the workflow?",
      topK: 1,
    });
  ...
  it("prefers media segments whose speaker metadata matches the query", async () => {
    ...
    const results = await collection.search({
      query:
        "What did Alex say about the workflow staying aligned across every frontend?",
      topK: 1,
    });
  ...
  it("prefers quoted speaker attribution matches in media reranking", async () => {
    ...
    const results = await collection.search({
      query:
        'What did "Alex K" say about the stable rollout staying aligned across every frontend?',
      topK: 1,
    });
  ...
  it("prefers quoted channel attribution matches in media reranking", async () => {
    ...
    const results = await collection.search({
      query:
        'What did Alex say on the "left" channel about the stable rollout staying aligned across every frontend?',
      topK: 1,
    });
  ...
  it("prefers continuous follow-up media segments for next/after queries", async () => {
    ...
    const results = await collection.search({
      query:
        "What does Alex say in the next left channel segment after the rollout update?",
      topK: 1,
    });
  ...
  it("prefers continuous prior media segments for before/previous queries", async () => {
    ...
    const results = await collection.search({
      query:
        "What did Alex say in the previous left channel segment before the rollout update?",
      topK: 1,
    });
  ...
  it("can apply MMR diversity reordering to reduce redundant vector hits", async () => {
    ...
    expect(
      traced.trace.steps.find((step) => step.stage === "diversity")?.metadata
        ?.mmrLambda,
    ).toBe(0.5);
  });
  ...
  it("treats transformed query variants as fallback candidates instead of co-equal primaries", async () => {
    const store = createInMemoryRAGStore({
      dimensions: 2,
      mockEmbedding: async (text) => (text.includes("00:00") ? [0, 1] : [1, 0]),
    });
    const collection = createRAGCollection({
      queryTransform: createHeuristicRAGQueryTransform(),
      store,
