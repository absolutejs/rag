...
const aiIndexPath = new URL("../../../../src/ai/rag/index.ts", import.meta.url)
  .href;
...
describe("RAG public export contract", () => {
  it("keeps workflow helpers off the root RAG export", async () => {
    const ai = await import(aiIndexPath);

    expect("createRAGWorkflow" in ai).toBe(false);
    expect("createRAGStream" in ai).toBe(false);
...
describe("RAG workflow wrapper parity", () => {
  ...
  it("angular workflow adds state on top of the stream contract", async () => {
    ...
    const { AIStreamService } = await import("@absolutejs/ai/angular");
    ...
    try {
      const { RAGWorkflowService } =
        await import("../../../../src/angular/ai/ai-rag-workflow.service");
  ...
  it("react search wrapper exposes trace-aware search", async () => {
    ...
    expect(await result?.searchWithTrace({ query: "alpha", topK: 1 })).toEqual(
      traceResponse,
    );
  ...
  it("svelte search wrapper exposes trace-aware search", async () => {
    ...
    expect(response).toEqual(traceResponse);
    expect(get(result.trace as Readable<unknown>)).toEqual(traceResponse.trace);
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("angular rag client service exposes trace-aware search", async () => {
    ...
    const { RAGClientService } =
      await import("../../../../src/angular/ai/rag-client.service");
