  ...
  it("supports OpenAI-compatible OCR base URLs", async () => {
    ...
    const provider = openaiCompatibleOCR({
      ...
      fetch: async (input) => {
        ...
        return new Response(JSON.stringify({ output_text: "compat ocr" }), {
          status: 200,
        });
      },
    });
  ...
  it("builds Ollama OCR requests for local multimodal models", async () => {
    ...
    const provider = ollamaOCR({
      ...
      fetch: async (input, init) => {
        ...
        return new Response(JSON.stringify({ response: "local ocr text" }), {
          status: 200,
        });
      },
    });
    ...
    expect(String(calls[0]?.input)).toBe("http://localhost:11434/api/generate");
    expect(JSON.parse(String(calls[0]?.init?.body))).toMatchObject({
      model: "llava",
      stream: false,
