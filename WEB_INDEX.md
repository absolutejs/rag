...
The final controlled 100-page lifecycle run measured 11.31 ms warm query p50 / 13.36 ms p95, 88.50 ms update-to-query visibility and 37.94 ms takedown-to-confirmation. It uses PostgreSQL 15, simulated public-page responses and deterministic vectors, not external embedding APIs. Run `benchmarks/web-index/lifecycle.ts` with a disposable database and `--output=NEW_PATH` to reproduce; do not generalize those numbers to public network/model latency.
