...
- **Replace backtracking HTML app-shell detection with DOM inspection and allow isolated reader clients to fail without in-process parsing** (`readRAGWebpage`, `createWebsiteReaderClient`, `createWebpageReaderClient`)

## 0.22.1 — 2026-09-24

### Fixed

- **Retry owned-index initialization after a transient database outage instead of retaining a rejected initialization promise** (`createWebIndex`)

## 0.22.0 — 2026-09-24

### Added

- **Add durable owned web indexes with PostgreSQL crawl leases, versioned evidence, native search-provider integration, authenticated plugins, workers, reviewed entity projections and six-framework clients** (`createWebIndex`, `createPostgresWebIndexStore`, `webIndexPostgresSchemaSql`, `webIndexPlugin`, `renderWebIndexStats`, `startWebIndexWorker`, `createWebIndexProjector`, `createWebIndexClient`, `bindWebIndexSearchForm`, `useWebIndex`, `createWebIndexStore`, `WebIndexService`)

### Fixed

- **Preserve surrounding evidence context, validate quoted wording, support extraction without planning, and adapt isolated single-page readers without mixing source attribution** (`createResearch`, `researchPlugin`, `createWebpageReaderClient`)
