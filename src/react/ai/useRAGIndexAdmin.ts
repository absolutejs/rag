...
export const useRAGIndexAdmin = (path: string) => {
  ...
  const [lastMutation, setLastMutation] = useState<RAGMutationResponse | null>(
    null,
  );
  const [backends, setBackends] = useState<RAGBackendsResponse | null>(null);
  const [syncSources, setSyncSources] = useState<RAGSyncResponse | null>(null);
  ...
  const run = useCallback(async <T>(operation: () => Promise<T>) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "RAG index administration failed";
      setError(message);
      throw err;
    } finally {
      setIsLoading(false);
