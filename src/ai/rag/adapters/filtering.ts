    ...
    switch (operator) {
      ...
      case "$in":
        return Array.isArray(expected)
          ? expected.some((candidate) => valuesMatch(candidate, actual))
          : false;
      case "$contains":
        return arrayContainsValue(actual, expected);
      case "$containsAny":
        return Array.isArray(expected)
          ? expected.some((candidate) => arrayContainsValue(actual, candidate))
          : false;
      case "$containsAll":
        return Array.isArray(expected)
          ? expected.every((candidate) => arrayContainsValue(actual, candidate))
          : false;
      case "$ne":
  ...
  switch (key) {
    case "$and":
      return isNestedFilterArray(value)
        ? value.every((entry) => matchesMetadataFilterRecord(record, entry))
        : false;
    case "$or":
      return isNestedFilterArray(value)
        ? value.some((entry) => matchesMetadataFilterRecord(record, entry))
        : false;
    case "$not":
