...
const buildExtractionFailureDiagnostics = (
  ...
  let retryGuidance: RAGSyncSourceDiagnostics["retryGuidance"] = {
    ...
    reason:
      "Inspect skipped source items and rerun sync after fixing the extraction issue.",
  };
  ...
  if (remediationKinds.has("configure_extractor")) {
    ...
    retryGuidance = {
      ...
      reason:
        "Register or enable an extractor for the skipped source items, then rerun sync.",
    };
  }
  ...
  if (remediationKinds.has("add_ocr_extractor")) {
    ...
    retryGuidance = {
      action: "configure_extractor",
      reason:
        "Add an OCR-capable PDF extractor for the skipped scanned documents, then rerun sync.",
    };
  }
...
const buildSyncSourceDiagnostics = (record: {
  ...
  if (
    ...
    retryGuidance ??= {
      action: "resume_sync",
      reason:
        "Run this storage sync again to continue the paged source walk before deleting stale documents.",
      resumeCursor,
    };
  }
  ...
  if (
    ...
    retryGuidance ??= {
      action: "resume_sync",
      reason:
        "Run this email sync again to continue the paged mailbox walk before deleting stale documents.",
      resumeCursor,
    };
  }
  ...
  if (record.reconciliation) {
    if (record.reconciliation.lineageConflicts.length > 0) {
      ...
      retryGuidance ??= {
        ...
        reason:
          "Resolve sync lineage conflicts before trusting stale/latest version cleanup.",
    ...
    if (
      ...
      entries.push({
        code: "noop_sync",
        severity: "info",
        summary: "No managed source changes were detected during this sync run",
      });
    }
  }
...
const reconcileManagedDocuments = async (input: {
  ...
  const lineageConflicts = [...managedDocumentsBySyncKey.entries()]
    .map(([syncKey, documents]) => {
      ...
      const latestDocuments = documents.filter(isDocumentSyncLatestVersion);
      const reasons: Array<
        | "duplicate_sync_key"
        | "multiple_lineages"
        | "multiple_versions"
      ...
      return {
        ...
        latestDocumentIds: latestDocuments.map((document) => document.id),
        lineageIds,
        reasons,
        syncKey,
        versionIds,
      ...
      metadata: {
        ...
        ...(exactVersion
          ...
          : {
              ...
              syncPreviousVersionId: getDocumentSyncVersionId(latestVersion),
            }),
      },
  ...
  const reconciliation: RAGSyncSourceReconciliationSummary = {
    ...
    targetedRefreshSyncKeys: [
      ...new Set([
        ...staleDocuments.map((document) => getDocumentSyncKey(document)),
  ...
  const latestKeeper =
    conflict.latestDocumentIds.length === 1
      ? conflict.documents.find(
          (document) => document.documentId === conflict.latestDocumentIds[0],
        )
      : undefined;
  const highestVersionDocuments = [...conflict.documents]
    .filter(
      (document): document is typeof document & { versionNumber: number } =>
...
export const previewRAGSyncConflictResolutions = (input: {
  ...
  const actions = reconciliation.lineageConflicts
    .map((conflict) => buildSyncConflictResolutionAction(conflict, strategy))
  ...
  return {
    ...
    strategy,
    unresolvedConflicts,
    unresolvedSyncKeys: unresolvedConflicts.map((conflict) => conflict.syncKey),
  };
};
...
const recoverSyncSourceRecord = (
  ...
  record.status === "running"
    ? toSourceRecord(source, {
        ...record,
        lastError:
          record.lastError ?? "Interrupted before completion during recovery",
  ...
  sync: async ({ collection, deleteDocument, listDocuments }) => {
    ...
    const includeExtensions =
      ...
      (options.extractors?.length || options.extractorRegistry)
        ? null
        : new Set(
            (options.includeExtensions ?? DEFAULT_DIRECTORY_EXTENSIONS).map(
              (entry) =>
                entry.startsWith(".")
                  ? entry.toLowerCase()
                  : `.${entry.toLowerCase()}`,
...
export const createRAGGitHubSyncSource = (
  ...
  sync: async ({ collection, deleteDocument, listDocuments }) => {
    ...
    const apiBaseURL =
      options.apiBaseUrl?.trim().replace(/\/$/, "") || "https://api.github.com";
        ...
        [...feedMap.values()].map(async (feed) => {
          ...
          const limited =
            typeof options.maxEntriesPerFeed === "number"
              ? entries.slice(0, Math.max(1, options.maxEntriesPerFeed))
              : entries;
        ...
        [...resolvedSitemapMap.values()].map(async (sitemap) => {
          ...
          const limited =
            typeof options.maxUrlsPerSitemap === "number"
              ? entries.slice(0, Math.max(1, options.maxUrlsPerSitemap))
              : entries;
          return limited.map((entry) => ({
            entry,
            sitemap,
            ...
            .filter((entry) => {
              const entrySiteURL =
                typeof entry.metadata?.siteUrl === "string"
                  ...
                  : undefined;
              return entrySiteURL ? entrySiteURL === site.url : true;
            })
            .map((entry) => entry.url),
        ];
      ...
      options.urls.map(async (urlInput) => {
        try {
          const document = await loadRAGDocumentFromURL({
            ...urlInput,
            extractorRegistry:
              urlInput.extractorRegistry ?? options.extractorRegistry,
  ...
  sync: async ({ collection, deleteDocument, listDocuments, sourceRecord }) => {
    const listed = await (async () => {
      let cursor =
        options.resumeFromLastCursor === false
          ? undefined
          : getSyncMetadataString(sourceRecord?.metadata, "resumeNextCursor");
...
export const createRAGSyncManager = (
  ...
  const runSource = async (
    ...
    const runPromise = (async () => {
      ...
      return (
        state.get(source.id) ?? toSourceRecord(source, { status: "failed" })
      );
    })().finally(() => {
