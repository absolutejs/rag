...
export const createRAGStream = (path: string, conversationId?: string) => {
  ...
  const sourceGroups = derived(workflow, ($workflow) => $workflow.sourceGroups);
  ...
  const hasRetrieved = derived(workflow, ($workflow) => $workflow.hasRetrieved);
  const hasSources = derived(workflow, ($workflow) => $workflow.hasSources);
  const isRetrieving = derived(workflow, ($workflow) => $workflow.isRetrieving);
