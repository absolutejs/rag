...
const scoreFeatureFamilyMatches = ({
  ...
  const matchCount = countDistinctTokenMatches(
    queryTokens,
...
const scoreStructuredEvidenceMatch = (
  ...
  const pdfTextKind =
    typeof metadata.pdfTextKind === "string" ? metadata.pdfTextKind : undefined;
  ...
  const emailAttachmentSource = metadataString(metadata.emailAttachmentSource);
  ...
  const emailMailboxFamilyKey = metadataString(metadata.emailMailboxFamilyKey);
  ...
  const emailClientSubmitTime = metadataString(metadata.emailClientSubmitTime);
  ...
  const emailLastModifiedTime = metadataString(metadata.emailLastModifiedTime);
  ...
  if (
    ...
    queryHasAnyToken(queryTokens, ["table", "tables", "sheet", "spreadsheet"])
  ) {
    score += 0.16;
    if (
  ...
  if (
    slideNotesText &&
    queryHasAnyToken(queryTokens, ["notes", "speaker", "speakers", "talking"])
  ) {
    score += 0.2;
  }
  ...
  if (
    ...
    if (
      ...
      const channelPhraseScore = scoreLoosePhraseMatch(query, mediaChannel);
  ...
  if (
    ...
    queryHasAnyToken(queryTokens, ["before", "previous", "prior", "earlier"])
  ) {
  ...
  if (
    ...
    queryHasAnyToken(queryTokens, ["thread", "reply", "replies", "attachment"])
  ) {
    score += 0.08;
  }
  ...
  const mailboxStateSignals = [
    {
      active: emailMailboxIsUnread || emailMailboxStateFlags.includes("unread"),
    ...
    {
      active:
        emailMailboxIsFlagged || emailMailboxStateFlags.includes("flagged"),
      queryTokens: ["flagged", "starred", "important"],
      score: 0.2,
    },
    {
      active:
        emailMailboxIsReplied || emailMailboxStateFlags.includes("replied"),
      queryTokens: ["replied", "answered"],
      score: 0.18,
    },
    {
      active: emailMailboxIsDraft || emailMailboxStateFlags.includes("draft"),
      queryTokens: ["draft"],
      score: 0.18,
    },
    {
      active:
        emailMailboxIsTrashed || emailMailboxStateFlags.includes("trashed"),
      queryTokens: ["trash", "trashed", "deleted", "bin"],
      score: 0.18,
    },
    {
      active: emailMailboxIsPassed || emailMailboxStateFlags.includes("passed"),
  ...
  const mailboxIdentitySignals = [
    {
      active:
        ...
        (prefersEmailNormalizedSubject ||
          queryTokens.some((token) => emailNormalizedSubject.includes(token))),
      score: 0.14,
    },
    {
      active:
    ...
    },
    {
      active:
        ...
        (prefersEmailConversationId ||
          queryTokens.some((token) => emailConversationId.includes(token))),
      score: 0.12,
    },
    {
      active:
        ...
        (prefersEmailConversationIndex ||
          queryTokens.some((token) => emailConversationIndex.includes(token))),
      score: 0.12,
    },
    {
      active:
    ...
    },
    {
      active:
        ...
        (prefersEmailMessageId ||
          queryTokens.some((token) => emailInternetMessageId.includes(token))),
      score: 0.14,
    },
    {
      active:
  ...
  const mailboxTimeSignals = [
    ...
    {
      active:
        (emailReceivedAt || emailDeliveryTime) && prefersEmailReceivedTime,
      score: 0.12,
    },
    {
  ...
  const mailboxFormatSignals = [
    {
      active:
        ...
        queryHasAnyToken(queryTokens, ["outlook"]),
      score: 0.08,
    },
    {
      active:
  ...
  const mailboxAncestrySignals = [
    ...
    {
      active: typeof replyDepth === "number" && prefersEmailThreadAncestry,
      score: 0.06 + scoreBoundedMagnitude(replyDepth, 0.05, 0.16),
    },
    {
      active: threadMessageIds.length > 0 && prefersEmailThreadAncestry,
      score: 0.04 + scoreBoundedMagnitude(threadMessageIds.length, 0.04, 0.18),
    },
    {
      active:
        typeof replyReferenceCount === "number" && prefersEmailThreadAncestry,
  ...
  if (emailSectionKind === "forwarded_headers") {
    ...
    if (typeof emailForwardedOrdinal === "number") {
      score -= Math.min(0.12, Math.max(0, emailForwardedOrdinal - 1) * 0.03);
    }
    if (
  ...
  if (
    ...
    score += scoreActiveSignals(
      mailboxStateSignals.map((signal) => ({
        active:
          signal.active && queryHasAnyToken(queryTokens, signal.queryTokens),
        score: signal.score,
      })),
    );
  }
  ...
  if (
    ...
    const lineageSources = [
      ...(emailMailboxContainerSource ? [emailMailboxContainerSource] : []),
  ...
  if (
    ...
    queryHasAnyToken(queryTokens, ["nested", "inner", "container", "archive"])
  ) {
    score += 0.12;
  }
...
const scoreHeuristicMatch = ({
  ...
  const sourcePathBoost =
    ...
    queryTokens.some((token) => result.source?.toLowerCase().includes(token))
      ? 0.5
      : 0;
  const metadataBoost =
    metadataValues.length > 0
      ? queryTokens.filter((token) =>
          metadataValues.some((value) => value.toLowerCase().includes(token)),
